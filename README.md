<img src="./banner.png" alt="banner" width="100%"/>

<h1 align="center">印贤聪 · Cong</h1>
<p align="center">
  <a href="https://ccccongggg.github.io"><img src="https://img.shields.io/badge/主页-Astro-056de8?style=flat-square&logo=astro" alt="site"/></a>
  <img src="https://img.shields.io/badge/专业-智能制造工程-blue?style=flat-square" alt="major"/>
  <img src="https://img.shields.io/badge/认证-PCB工程师-green?style=flat-square" alt="pcb"/>
  <img src="https://img.shields.io/badge/坐标-济南·广州-orange?style=flat-square" alt="loc"/>
  <img src="https://img.shields.io/badge/状态-备赛中-red?style=flat-square" alt="status"/>
</p>

> 🔧 智能制造工程本科在读 · 认证 PCB 工程师 ｜ 硬件 · 嵌入式 · 三维设计 ｜ 以赛代练，把想法做成实物

---

### 👋 关于我

- 🎓 **齐鲁理工学院 · 智能制造工程**（本科在读）
- 🛠️ **三栖工程**：硬件电路 / 嵌入式（ESP32 · FPGA · C · Rust）/ 机械三维（SOLIDWORKS 等级 4）
- 🏆 机电创新省二等奖、成图大赛省级奖；千人黑客松硬件赛道入围
- 🌱 **正在做**：Insta360 Link 2 云台控制平台 `link2_studio`、YOLO11 目标检测部署、Link 2 便携供电设计
- 💡 偏好用 **Web（Three.js / WebSocket / Astro）** 给产品加功能，而非传统嵌入式路线

### 🧰 技术栈

<img src="https://skillicons.dev/icons?i=cpp,c,python,rust,arduino,raspberrypi,autocad,solidedge,html,css,javascript,vue,threejs,linux,git,docker&perline=9" alt="tech stack"/>

### 🚀 精选项目

| 项目 | 说明 | 技术 |
|------|------|------|
| [场景炼金师](https://github.com/ccccongggg/changjing-scene-alchemist) | 知乎好帖「同帖千面」真实场景迁移工具（Zhihu Hackathon） | Vue3 |
| [Scenario Alchemist](https://github.com/ccccongggg/scenario-alchemist) | 星云图 / 星座信息架构原型 | HTML / JS |
| [Focus Rover](https://github.com/ccccongggg/focus-rover) | 专注看板机器人 · 三人小队立项页 | HTML |
| [齐鲁理工实验室安全 Skill](https://github.com/ccccongggg/qilu-lab-safety-skill) | SkillHub 宝藏母校大赛 · 实用刚需赛道 | Python |
| [个人主页](https://ccccongggg.github.io) | Astro 5 + Sveltia CMS + EdgeOne Pages | Astro |

### 📊 GitHub 数据

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=ccccongggg&show_icons=true&theme=tokyonight&count_private=true&hide_rank=false" height="170"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ccccongggg&layout=compact&theme=tokyonight&hide=html,css" height="170"/>
</p>
<img src="https://github-readme-activity-graph.vercel.app/graph?username=ccccongggg&theme=tokyonight&hide_border=true&area=true" width="100%"/>

### 🏅 竞赛 & 荣誉

- 🥈 机电创新大赛 · 省级二等奖
- 🏆 成图大赛 · 省级奖
- 🤖 烈变·She Nicest 千人黑客松 · 硬件赛道入围奖（作品《伊宝·电子手办》）
- 🔥 第十二届 3S 杯物联网大赛、NVIDIA DGX Spark Agent Skills 挑战赛 · 参赛中

### 📫 联系我

- 🌐 主页 / 博客：<https://ccccongggg.github.io>
- 💻 GitHub：[@ccccongggg](https://github.com/ccccongggg)

---

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=ccccongggg&label=Profile%20Views&color=blueviolet&style=flat-square" alt="visitors"/>
</p>
